/**
 * @api {post} /dedup-analyzer
 * @apiName DedupAnalyzer
 * @apiGroup Edge Functions
 *
 * @apiDescription
 * This edge function handles duplicate detection analysis and statistics retrieval.
 * It requires a valid Supabase JWT for authentication.
 *
 * @apiParam (Body) {String} action The action to perform. Either 'analyze_uploaded_files' or 'get_dedup_stats'.
 * @apiParam (Body) {String} [user_id] The ID of the user. Required for 'analyze_uploaded_files'.
 * @apiParam (Body) {String[]} [file_ids] An array of file IDs to analyze. Required for 'analyze_uploaded_files'.
 *
 * @apiSuccess (analyze_uploaded_files) {Boolean} success Indicates if the analysis was successful.
 * @apiSuccess (analyze_uploaded_files) {Number} total_files The total number of files analyzed.
 * @apiSuccess (analyze_uploaded_files) {Number} duplicates_found The number of duplicate files found.
 * @apiSuccess (analyze_uploaded_files) {Object[]} duplicate_groups An array of groups of duplicate files.
 * @apiSuccess (analyze_uploaded_files) {Number} processing_time_ms The time taken for the analysis in milliseconds.
 * @apiSuccess (analyze_uploaded_files) {String} analysis_timestamp The timestamp of the analysis.
 * @apiSuccess (analyze_uploaded_files) {Number} potential_savings_bytes The potential storage savings in bytes.
 *
 * @apiSuccess (get_dedup_stats) {Boolean} success Indicates if the statistics retrieval was successful.
 * @apiSuccess (get_dedup_stats) {Object[]} recent_analyses An array of recent analysis events.
 * @apiSuccess (get_dedup_stats) {Number} total_analyses The total number of analyses performed.
 *
  ...
  try {
    ...
    switch (action) {
      case 'analyze_uploaded_files':
        ...
        const duplicateGroups: { id: string; files: any[]; similarity: number; size_bytes: any; count: number; }[] = [];

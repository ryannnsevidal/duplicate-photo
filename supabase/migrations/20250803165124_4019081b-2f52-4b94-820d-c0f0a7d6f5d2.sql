...
-- 1. Update OTP expiry to recommended value (1 hour = 3600 seconds)
UPDATE auth.config 
SET 
  sms_otp_exp = 3600,
  email_otp_exp = 3600
...
-- 2. Create admin user assignment function
...
-- 3. Assign admin role to your email (update this with your actual email)
-- SELECT public.assign_admin_role('rsevidal117@gmail.com');

-- 4. Create function to get user role efficiently
...
-- 5. Update RLS policies to be more restrictive (remove anonymous access)
...
-- 6. Create CAPTCHA verification table
...
-- 7. Create function to verify CAPTCHA

...
export function AdminCloudUpload({ onUploadComplete }: AdminCloudUploadProps) {
  ...
  const confirmUpload = async () => {
    ...
    for (const fileData of pendingFiles) {
      if (fileData.source === 'local') {
        await uploadLocalFile(fileData);
      } else {
        await uploadCloudFile(fileData, fileData);
      }
    }
  ...
  // Upload local file
  const uploadLocalFile = async (fileData: any) => {
    try {
  ...
  // Upload cloud file  
  const uploadCloudFile = async (uploadFile: any, cloudFile: any) => {
    setUploading(true);

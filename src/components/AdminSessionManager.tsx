...
export function AdminSessionManager() {
  ...
  const fetchActiveSessions = useCallback(async () => {
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Could not fetch active sessions.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const terminateSession = async (sessionId: string, userEmail: string) => {
    try {
      ...
      toast({
        ...
        description: error instanceof Error ? error.message : "Could not terminate the session.",
        variant: "destructive",
      });
    } finally {
      setTerminating(null);
  ...
  const getSessionStatus = (session: ActiveSession): { status: string; color: 'default' | 'secondary' | 'destructive' | 'outline' } => {
    const now = new Date();
                ...
                {sessions.map((session) => {
                  ...
                  return (
                    <TableRow key={session.id}>
                      ...
                      <TableCell>
                        <Badge variant={sessionStatus.color}>

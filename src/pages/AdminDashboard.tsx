...
export function AdminDashboard() {
  ...
  useEffect(() => {
    if (!loading && !user) {
      console.log('🔒 Admin dashboard: No user found, redirecting to signin');
      navigate('/signin');
      return;
    }
    ...
    if (user && isAdmin) {
      console.log('✅ Admin dashboard: Admin user verified, loading dashboard');
      fetchDashboardData();
    }
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-primary/5">
        ...
        >
          ...
          <p className="text-muted-foreground">
            {loading ? 'Verifying admin credentials...' : 'Loading admin dashboard...'}
          </p>
        </motion.div>
      </div>
    );
  ...
  // If not admin after loading, show access denied (this is a fallback, navigation should happen in useEffect)
  if (!loading && user && !isAdmin) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-background to-primary/5">
        <motion.div 
